...
*不同模型支持的视频生成能力简介
*Seedance 1.5 pronew 有声视频(自定义是否包含音频)
...
*图生视频-参考图，根据您输入的参考图片（1-4张）+文本提示词（可选）+ 参数（可选）生成目标视频。
*图生视频-首尾帧，根据您输入的首帧图片+尾帧图片+文本提示词（可选）+参数（可选）生成目标视频。
*图生视频-首帧，根据您输入的首帧图片+文本提示词（可选）+参数（可选）生成目标视频。
...
不同模型，可能对应支持不同的参数与取值，详见 输出视频格式 。当输入的参数或取值不符合所选的模型时，内容会被忽略或报错。
*示例
* 
*
...
ratio string 简写 rt
文生视频默认值一般是16:9
图生视频默认值一般是adaptive。特别注意，参考图生视频的默认值是16:9
Seedance 1.5 pro 默认值是adaptive
...
注意
Seedance 1.5 pro 文生视频设置adaptive时，表示根据用户提示词，智能选择最合适的宽高比。实际生成视频的宽高比可通过 查询视频生成任务 API 返回的 ratio 字段获取。
不同宽高比对应的宽高像素值
